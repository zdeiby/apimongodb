...
const entradaFecha=document.querySelector('.entrada_fecha')
let motoPrecio = '';
let mensajeMoto = ''
let carroPrecio = '';
let mensajeCarro = ''
...
async function mostrarFactura() {
  ...
  if (value == 1 && checkout == "INGRESADO" && mensualidades=="no") {
    ...
    const showText = `
            ...
            <label class="label_horas">Esta placa está ingresada en operaciones, por favor retirala antes de ingresarla acà</label><br>
                         `
    div.innerHTML = showText;
    div_totales.appendChild(div);
  } 
...
const date1=entradaFecha.value.replace('-', '/')
...
fechaEntrada=newfecha.toLocaleDateString()
  ...
  let horasde= new Date()
  let horaEntrada = new Date(horasde);
  let fechaEntradaString = horaEntrada.toLocaleDateString()
    fechaEntrada=fechaEntradaString
}
